...
export default function CertificateCardCertify({
  certificate,
  onClickProcess,
}) {
    ...
    >
      <div className="flex items-center gap-x-[5px] w-full">
        <CertificateImage
          ...
          image={certificate?.courseImg || defaultCourseImage}
        />
      ...
      <Collapse in={expanded} timeout="auto" unmountOnExit>
        <CardContent>
          <Typography paragraph>
            <span className="text-base font-semibold">Ngày đăng kí:</span>{" "}
            {moment(certificate.enrollDate).format("DD/MM/YYYY")}
          </Typography>
          <Typography paragraph>
            <span className="text-base font-semibold">Ngày nộp:</span>{" "}
            {moment(certificate.submitDate).format("DD/MM/YYYY")}
          </Typography>
          <TextField
        ...
        <Stack direction="row" sx={{ float: "right", pb: 3, mr: 2 }}>
          <IconButton
            aria-label="Từ chối"
            onClick={() => onClickProcess(false, certificate)}
          >
